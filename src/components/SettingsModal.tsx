        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 pr-3">
          ...
          {activeTab === 'profile' && (
            <div className="space-y-5">
              {/* Firebase Google / Email Account Status Banner */}
              <div className="p-4 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 space-y-3 relative overflow-hidden">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      {currentUser?.photoURL ? (
                        ...
                        <div className="w-11 h-11 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 font-bold text-sm">
                          {currentUser ? (
                    ...
                    <div>
                      <div className="text-xs font-bold text-white flex items-center gap-1.5">
                        <span>{currentUser ? currentUser.displayName || 'Pengguna Terverifikasi' : 'Mode Tamu (Offline)'}</span>
                        {currentUser && (
                      ...
                      <div className="text-[11px] text-slate-400 truncate max-w-xs">
                        {currentUser ? currentUser.email : 'Belum masuk akun Google / Email'}
                      </div>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-300 border border-emerald-500/40 text-xs font-semibold transition-all active:scale-95"
                  >
                    {currentUser ? 'Kelola Akun' : 'Masuk Akun'}
                  </button>
                </div>
                ...
                {currentUser && (
                  <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between text-xs">
                    <div className="flex items-center gap-1.5 text-slate-400">
                      ...
                      <span>
                        {lastSyncedTime
                          ? `Sinkron: ${new Date(lastSyncedTime).toLocaleTimeString('id-ID', {
                              hour: '2-digit',
                              minute: '2-digit',
                            })}`
                    ...
                    {onManualCloudSync && (
                      <button
                        ...
                        className="text-[11px] text-slate-300 hover:text-white flex items-center gap-1 transition-colors disabled:opacity-50"
                      >
                        <RefreshCw className={`w-3 h-3 ${isCloudSyncing ? 'animate-spin text-emerald-400' : ''}`} />
