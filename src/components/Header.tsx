        ...
        <div className="flex items-center gap-2">
          ...
          {onOpenAuthModal && (
            <button
              ...
              className={`flex items-center gap-2 p-1.5 sm:px-3 sm:py-1.5 rounded-xl border transition-all active:scale-95 text-xs font-semibold ${
                currentUser
                  ? 'bg-slate-800/90 hover:bg-slate-750 border-emerald-500/30 text-white'
                  : 'bg-emerald-500/10 hover:bg-emerald-500/20 border-emerald-500/40 text-emerald-300'
              }`}
              title={currentUser ? `Akun: ${currentUser.email || currentUser.displayName} (Klik untuk info sync)` : 'Masuk via Google atau Email'}
            >
              {currentUser ? (
                <>
                  <div className="relative">
                    {currentUser.photoURL ? (
                      ...
                      <div className="w-6 h-6 rounded-full bg-emerald-600/30 border border-emerald-500/40 text-emerald-400 font-bold flex items-center justify-center text-[10px]">
                    ...
                    <span
                      ...
                      title={isCloudSyncing ? 'Sedang sinkronisasi...' : 'Tersinkron ke Cloud'}
                    />
                  </div>
                ...
                <>
                  ...
                  <span className="hidden sm:inline font-bold">Masuk / Login</span>
                </>
              )}
            </button>
          )}
          ...
          {onOpenReceiptScan && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-emerald-300 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 rounded-xl transition-all shadow-sm active:scale-95"
              title="Foto Nota / Struk Belanja Otomatis (OCR)"
            >
              <Camera className="w-4 h-4 text-emerald-400" />
              <span className="hidden sm:inline">Foto Nota</span>
